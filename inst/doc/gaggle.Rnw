...
do justice to these data.  In fact -- and despite significant effort having been devoted
to integrating many kinds of data within single programs and web sites in recent years-- 
the heterogeneity of biological data is only likely to increase, along with the number
and variety of programs and web sites used to view them.
...
commonly used
in bioinformatics (and, indeed, in many other scientific fields). They are
...
The four Gaggle \textbf{data types} are distilled versions of data types commonly used
in bioinformatics (and, indeed, in many other scientific fields). These data types are
essentially free of biological semantics:
R data types they are mapped to:
...
\begin{center}
\url{http://gaggle.systemsbiology.org/R/vignette/1}
\end{center}
...
As of this writing (April 2006) web resources -- bioinformatics
...
\begin{itemize}
\item Start the Gaggle Boss  (browse to \url{http://gaggle.systemsbiology.org/R/vignette/1}, Part 1, for web start links)
...
Inasmuch as the R goose does not hae a full-fledged graphical user interface,
